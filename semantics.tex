...
Each healable $\tau$ has exactly two rules: $\rulename{Gen-Ok}_\tau$ fires when both $v$ and the continuation type-check, stepping to $\Inst{p}{v}$ and flushing $\MErr$; $\rulename{Gen-Heal}_\tau$ fires when only $v$ does, extending $\MErr$ with the continuation's $\epsilon$ and retrying. Two uniform rules apply at every $\tau$: $\rulename{Gen-TypeErr}$ steps an ill-typed response at non-healable $\tau$ to the fail-closed sink $\errV$ (intentionally untypeable: $\TJok{\Gctx}{\errV}{\tau}$ is unprovable for every $\tau$), and $\rulename{Gen-PolHeal}$ records a feedback event and retries on policy denial. \figr{fig:rules-core} shows the Schema pair; the arrow pair is structurally identical with $\tau = \Tarrow{T_1,\ldots,T_n}{T'}$ and is given in Appendix~\ref{app:rules}. For arrows, $\Inst{p}{v}$ is higher-order substitution of the oracle-returned closure into the continuation, mechanized via the \texttt{lean-subst} framework. The full rule set --- including the $\rulename{Gen-Budget}$ sink and the Policy pair, which is subsumed by the parametric $\rulename{Healable-Ok}$/$\rulename{Healable-Heal}$ rules via $\Healable{\Tpolicy} = \mathsf{true}$ --- appears in Appendix~\ref{app:rules}.
...
\end{mathpar}
\caption{Self-healing for the Schema materialization target. Both rules require a well-typed oracle response ($\TJok{\Gctx}{v}{\Tschema}$); they differ in whether the \emph{continuation} $p$ type-checks under $\Gext{x}{\Tschema}$. $\rulename{Schema-Ok}$ flushes the heal context to $\emptyset$ and commits; $\rulename{Schema-Heal}$ appends the continuation's feedback $\epsilon$ and retries the same redex. If the oracle returns an ill-typed $v$, no rule fires --- the configuration is stuck by omission, and the progress lemma (T4c) discharges this case under an eventually-truthful oracle. The arrow (workflow) pair is structurally identical with $\tau = \Tarrow{T_1,\ldots,T_n}{T'}$ (Appendix~\ref{app:rules}); the parametric pair $\rulename{Healable-Ok}$/$\rulename{Healable-Heal}$, given in the appendix, generalizes both pairs to any $\Healable{\tau}$ and additionally covers nested compound healable types such as $\Tarr{\Tschema}$ (clinical-trial \texttt{Array[crf]}) and $\Tpolicy$ (whose segment-authorization check is the existing $\PolOk{}{}{\texttt{gen}}$ premise of the parametric rule).}
...
{\sloppy Let $C = \MConfigS{\MErr}{\MPol}{\MStore}{e}$ with $\WF{C}$. \textbf{T4a} (no heal past budget for standalone gen): if $e = \Gen{\tau}{s}{\MPrin}$ and $\retries{\MErr} > \MRetry$, then $C$ has no successor at all (standalone $\Gen{\tau}{s}{\MPrin}$ is not a redex). \textbf{T4b} (truthful success at Schema): if $e = \Let{\Tschema}{\Gen{\Tschema}{s}{\MPrin}}{\mathrm{var}\;0}$, $\PolOk{\MPol}{\MPrin}{\texttt{gen}}$, and $\mathcal{O}$ is eventually-truthful at $(s,\Tschema)$, then $\exists C'.\ C \step C'$ via $\rulename{Schema-Ok}$. \textbf{T4c} (progress-gen-Schema, progress-agent): with the same authorization hypothesis and a direct oracle witness at the caller's $\MErr$, $\exists C'.\ C \step C'$.\par}
\end{theorem}
