...
-/
inductive PureStep (O : Oracle) : Config → Config → Prop where
  | var {ρ ec P π x v b}
      (h : Env.lookup ρ x = some b)
      (hv : b.value = v) :
      PureStep O ⟨ρ, ec, P, π, .var x⟩ ⟨ρ, ec, P, π, .valE v⟩
  ...
  | jsStep {ρ ec P π je v}
      ...
      PureStep O ⟨ρ, ec, P, π, .js je⟩ ⟨ρ, ec, P, π, .valE v⟩
...
inductive PureSteps (O : Oracle) : Config → Config → Prop where
  | refl {C} : PureSteps O C C
  | step {C C' C''} : PureStep O C C' → PureSteps O C' C'' → PureSteps O C C''
