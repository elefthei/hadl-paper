...
The design of \pact{} deliberately trades expressiveness for formal 
tractability, and several of those trade-offs open concrete directions 
for future work. The path-insensitive analysis, the restricted type 
structure, and the sequential execution model each represent a 
principled simplification that could be relaxed without abandoning 
the core guarantees. We discuss these promising extensions below.
...
Host-language expressions (arithmetic, field projections, comparisons) 
execute arbitrary JavaScript under the host runtime's ambient 
authority via the embedded Boa engine. The current type system treats 
these expressions as oracular, accepting whatever value the JavaScript 
engine produces without modelling its side effects. This means 
that a host-language expression can perform file I/O, network requests, 
...
that tracks which capabilities these expressions exercise would 
